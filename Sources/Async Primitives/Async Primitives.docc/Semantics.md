...
| ``Async/Channel/Rendezvous`` | `send(_:)` and `receive()` throw ``Async/Channel/Error/cancelled`` when that suspended operation is withdrawn. Close is terminal: pending and future sends throw `.closed`, while pending and future receives return `nil` | Sender and receiver waiters each form an independent FIFO queue. A match removes the front eligible waiter from the opposite queue | **Unbuffered** — a send completes only after one receiver accepts its element | **FIFO admission on both sides**. Resume-call order is receiver then sender; observable task-completion order remains scheduler-determined |
