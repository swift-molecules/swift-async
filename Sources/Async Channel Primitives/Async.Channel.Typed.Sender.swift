...
// ===----------------------------------------------------------------------===//
    ...
    extension Async._Channel.Typed where Element: ~Copyable, Failure: Swift.Error & Sendable {
        ...
        public struct Sender: Sendable {
            @usableFromInline let raw: Async._Channel<Element>.Bounded.Sender
            @usableFromInline let terminals: TerminalStorage

            @usableFromInline
            init(raw: Async._Channel<Element>.Bounded.Sender, terminals: TerminalStorage) {
                self.raw = raw
    ...
    extension Async._Channel.Typed.Sender where Element: ~Copyable, Failure: Swift.Error & Sendable {
